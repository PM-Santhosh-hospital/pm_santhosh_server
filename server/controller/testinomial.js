...
exports.createTestinomial = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ msg: "Testinomial Created Successfully", testinomial });
  } catch (error) {
    console.log(error);
...
exports.deleteTestinomial = async (req, res) => {
  try {
    const testinomial = await Testinomial.findByIdAndDelete({
      _id: req.params.id,
    });
    res
      .status(201)
      .json({ msg: "Testinomial deleted Successfully", testinomial });
  } catch (error) {
    res.status(401).json({ err: "Something went Wrong!!", error });
  }
};
